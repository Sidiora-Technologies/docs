...
```bash
evmosd add-genesis-account my_validator 10000000000ahpx
```
...
evmosd add-genesis-account my_validator 1000000000stake,10000000000ahpx
```
...
```bash
evmosd start --evm.tracer $TRACER --tracestore $TRACESTORE --pruning=nothing $TRACE --log_level $LOGLEVEL --minimum-gas-prices=0.0001ahpx --json-rpc.api eth,txpool,personal,net,debug,web3
```
...
Data for the `evmosd` binary should be stored at `~/.evmosd`,
